      ...
      <div class="border rounded-md p-4 max-w-sm mx-auto">
        <div class="flex justify-between items-center border-2 border-spacing-1 rounded-md mb-4">
          <div class="flex justify-between">
            <div class="flex items-center w-1/3">
              <select class="select" name="" id="" v-if="country != null" @change="updateCurrentCurrency">
                <optgroup v-for="(c, k) in country" :key="k" :label="k">
                  <option v-for="(cc, kk) in c" :key="kk" :selected="kk == current_currency">
                    {{ kk }}
                  </option>
                </optgroup>
              </select>
            ...
            <input type="number" id="default" class="input text-right w-1/2" value="1" @keyup="updateValue" />
          </div>
        </div>
        ...
        <div v-if="currency.length != 0">
          <div class="flex justify-between items-center border rounded-md mb-4" v-for="(x, l) in currency" :key="l"
            ...
            <div class="delete">
              <button class="btn btn-ghost btn-sm" @click="deleteCurrency(x)">X</button>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-between mt-4">
          <label class="btn btn-sm rounded modal-button" for="show_country" @click="getCountry">
            + Add Currency
          </label>
    ...
    <div class="modal modal-bottom sm:modal-middle cursor-pointer">
      <div class="modal-box min-h-screen">
        <div class="sticky top-0 bg-base-100">
          ...
          <div class="mt-2">
            <input type="text" class="input w-full" placeholder="Search" id="search" autocomplete="off"
              @keyup="search" />
          </div>
        </div>
        <div v-if="country != null" class="mt-4">
          <div v-for="(c, k) in country" :key="k" class="border-b mb-2">
            ...
            <div v-for="(cc, kk) in c" :key="kk" class="p-2 currency_parent">
              ...
              <label for="show_country" class="flex justify-between currency" :id="kk" @click="addCurrency(kk)" v-else>
                <span class="mr-2">{{ kk }}</span>
                <span>-</span>
                <span class="flex-1 ml-2">{{ cc }}</span>
              </label>
